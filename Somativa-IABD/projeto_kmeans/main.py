...
def carregar_dados(caminho_csv: str) -> pd.DataFrame:
    """
    Lê o arquivo CSV e padroniza os nomes das colunas.
    Espera encontrar, mesmo que com acentos, algo equivalente a:
    - nome ação
    - preço ação R$
    - qtde cotas
    ...
    # Cria um dicionário de renome baseado nas colunas do arquivo oficial
    colunas = {c: c.strip().lower() for c in df.columns}

    # Renomeia de forma mais simples para usar no restante do código
    mapa_renome = {}
    for col_original, col_norm in colunas.items():
        if "nome" in col_norm and "acao" in col_norm:
            mapa_renome[col_original] = "nome_acao"
        elif ("preco" in col_norm or "preço" in col_norm) and "r$" in col_norm:
            mapa_renome[col_original] = "preco_acao"
        elif "qtde" in col_norm or "cotas" in col_norm:
            mapa_renome[col_original] = "qtde_cotas"
        elif "mercado" in col_norm and "bilhoes" in col_norm:
            mapa_renome[col_original] = "valor_mercado_bilhoes"
